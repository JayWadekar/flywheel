...
def to_file_snr_opt(SNR_opt, out_path, modes, suff='', verbose=True):
    
    for mode in modes:
        fname_out = os.path.join(out_path, 'snrs_'+mode+suff+'.txt')
        np.savetxt(fname_out, SNR_opt[mode])

def from_file_snr_opt(out_path, modes, suff='', verbose=True):
    SNRs = {}
    for mode in modes:
        fname_out = os.path.join(out_path, 'snrs_'+mode+suff+'.txt')
        if verbose:
            print('Loading SNRs for mode %s from file: %s' %(mode, fname_out))
        SNRs[mode] = np.loadtxt(fname_out)
...
def main(idx, FLAGS):
        ...
        snr_simulator = tsCompute.modeSNR_ratio(Net, 
                                                is_ASD=FLAGS.is_ASD, 
                                                fmin=FLAGS.fmin,
        ...
        for it in range( FLAGS.all_n_it_pools[idx] ):
            ...
            if FLAGS.resume_run>0:
                if os.path.exists(os.path.join(FLAGS.fout, 'snrs_22_net'+suffstr+'.txt')):
                    print('Batch already present, skipping...')
                    continue
            
            print('\nIn this chunk we have %s events, from %s to %s' %(nevents_chunk, i_in,  i_f  ))
            
            SNR_sq_opt_chunk_net = {mode:np.zeros(nevents_chunk) for mode in modes_used}
            SNR_sq_opt_chunk_ref = {mode:np.zeros(nevents_chunk) for mode in modes_used}
            
            for iev in range(nevents_chunk):
                tmpev = {k: ev_chunk[k][iev] for k in ev_chunk.keys()}
                tmpres = snr_simulator(tmpev, wf_model, df=1./8.)
                for mode in modes_used:
                    SNR_sq_opt_chunk_net[mode][iev] = np.sqrt(tmpres['net'][mode])
                    SNR_sq_opt_chunk_ref[mode][iev] = np.sqrt(tmpres[FLAGS.reference_detector][mode])

            to_file_snr_opt(SNR_sq_opt_chunk_net, FLAGS.fout, modes_used, suff='_net'+suffstr, verbose=True)
            to_file_snr_opt(SNR_sq_opt_chunk_ref, FLAGS.fout, modes_used, suff='_'+FLAGS.reference_detector+suffstr, verbose=True)
...
parser = argparse.ArgumentParser(prog = 'compute_mode_snr_ratios_from_catalog.py', description='Executable to run ``SNR_timeseries`` on a catalog of events, with the possibility to parallelize over multiple CPUs, ready to use both on single machines and on clusters.')
...
if __name__ =='__main__':
    ...
    if not 'ra' in events_loaded.keys():
        events_loaded['ra']  = events_loaded['phi']
        events_loaded['dec'] = 0.5*np.pi - events_loaded['theta']

        _ = events_loaded.pop('theta', None)
        _ = events_loaded.pop('phi', None)
    ...
    pin=FLAGS.idx_in
    SNR_opt_net = {mode: np.zeros(nevents_total) for mode in FLAGS.modes_list} 
    SNR_opt_ref = {mode: np.zeros(nevents_total) for mode in FLAGS.modes_list}
    ...
    for it in range(all_batch_sizes.shape[0]): # iterations
        for p in range(all_batch_sizes.shape[-1]): # pools
            ...
            if pf>pin:
                ...
                tmpres_net = from_file_snr_opt(FLAGS.fout, FLAGS.modes_list, suff='_net'+suff_batch)
                tmpres_ref = from_file_snr_opt(FLAGS.fout, FLAGS.modes_list, suff='_'+FLAGS.reference_detector+suff_batch)
                for mode in FLAGS.modes_list:
                    SNR_opt_net[mode][pin:pf] = tmpres_net[mode]
                    SNR_opt_ref[mode][pin:pf] = tmpres_ref[mode]

                pin = pf
    
    to_file_snr_opt(SNR_opt_net, FLAGS.fout, FLAGS.modes_list, suff='_net'+suffstr, verbose=True)
    to_file_snr_opt(SNR_opt_ref, FLAGS.fout, FLAGS.modes_list, suff='_'+FLAGS.reference_detector+suffstr, verbose=True)
        ...
        for it in range(all_batch_sizes.shape[0]): # iterations
                for p in range(all_batch_sizes.shape[-1]): # pools
                    ...
                    if pf>pin:
                        ...
                        try:
                            for mode in FLAGS.modes_list:
                                os.remove(os.path.join(FLAGS.fout, 'snrs_'+mode+'_net'+suffstr+'.txt'))
                                os.remove(os.path.join(FLAGS.fout, 'snrs_'+mode+'_'+FLAGS.reference_detector+suffstr+'.txt'))
                        except:
